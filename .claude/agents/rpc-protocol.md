...
You are a specialist in the JSON-RPC 2.0 wire protocol used by this project.
...
4. **Request vs notification.** Requests have an `id` and expect a response.
   Notifications do not. Document which is which when you add a method.
...
3. Document direction (TS→Go or Go→TS), kind (request or notification),
   and shape in `CLAUDE.md` under "Where things live"
4. If it's user-visible, surface it in the webview through `ChatPanel.ts`

## Debugging desync

If the codec gets stuck, the usual cause is unframed output on the channel.
Steps:
1. Add `console.error` on the TS side to dump raw stdin chunks from the Go process
2. Check Go code for `fmt.Print*` to stdout (should be `log.Print*` which goes to stderr)
3. Verify `Content-Length` matches the actual byte length of the JSON body (UTF-8)

## Things you do not do

- Do not change framing format
- Do not introduce a new transport (HTTP, websockets) without explicit approval
- Do not add provider-specific message shapes — keep the protocol transport-neutral
