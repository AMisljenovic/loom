...
4. Verify each VSIX is under 25 MB and contains exactly one binary
   (its own).
5. Print a summary:
   - VSIX files produced and their sizes
   - Total artifact size
   - Anything that looks off (oversized binaries, missing platforms)
6. Do **not** run `vsce publish`. Stop after producing the artifacts
   and report the suggested next commands to publish to Marketplace.

If any step fails, stop and report the failure clearly with the
