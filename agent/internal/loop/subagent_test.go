...
func TestResearchPresetAllowedTools(t *testing.T) {
	preset, err := PresetFor("research")
	if err != nil {
		t.Fatalf("PresetFor(research): %v", err)
	}
...
func TestSpawnSubAgentDepthLimit(t *testing.T) {
	...
	out := driver.execSpawnSubAgent(context.Background(), "grandchild", "conv", input)
...
func TestSpawnSubAgentRequiresTaskAndContext(t *testing.T) {
	...
	out := driver.execSpawnSubAgent(context.Background(), "root", "conv", mustJSON(t, map[string]string{"type": "research", "context": "ctx"}))
	if out.err == nil || !strings.Contains(out.err.Error(), "task is required") {
		t.Fatalf("expected task validation error, got %#v", out.err)
	}

	out = driver.execSpawnSubAgent(context.Background(), "root", "conv", mustJSON(t, map[string]string{"type": "research", "task": "task"}))
